...
func TestAssignedTasksFetchesRunnableStatuses(t *testing.T) {
	...
	var requests int
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/downloads/tasks/assigned" {
			t.Fatalf("unexpected path: %s", r.URL.Path)
		}
		requests++
		w.Header().Set("Content-Type", "application/json")
...
func TestClientErrorResponsesIncludeProblemBody(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "GET /api/downloads/tasks/assigned failed") || !strings.Contains(err.Error(), "not authorized") {
