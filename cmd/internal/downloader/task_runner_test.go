...
func TestCancelRunningUsesCauseForControlState(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.state, func(t *testing.T) {
			w := NewTaskRunnerWithAPI(config.Config{MaxConcurrentTasks: 5}, &recordingAPI{})
...
func TestTickUsesHeartbeatNextPollInterval(t *testing.T) {
	...
	w := NewTaskRunnerWithAPI(config.Config{PollInterval: time.Second}, api)
	w.logger = slog.New(slog.NewTextHandler(io.Discard, nil))
...
func TestTerminalDownloadStopsPreservePartialFiles(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			eng := &recordingEngine{
				downloadFunc: func(ctx context.Context, task DownloadTask, progress ProgressReporter) (Result, error) {
					...
					ready <- struct{}{}
					if tc.failedMessage != "" {
						return Result{}, errors.New(tc.failedMessage)
					}
					<-ctx.Done()
					return Result{}, ctx.Err()
				},
				resetTaskFn: func(context.Context, DownloadTask) error {
					return os.RemoveAll(filepath.Join(downloadDir, "task-1"))
				},
			}
			w := NewTaskRunnerWithAPI(config.Config{DownloadDir: downloadDir}, api)
			w.logger = slog.New(slog.NewTextHandler(io.Discard, nil))
			w.downloader = NewManagerWithDownloader(eng)
...
func TestClearStaleSeedingReportsClearsUntrackedOnly(t *testing.T) {
	...
	w := NewTaskRunnerWithAPI(config.Config{SeedEnabled: true}, api)
	w.logger = slog.New(slog.NewTextHandler(io.Discard, nil))
	w.downloader = NewManagerWithDownloader(&recordingEngine{})
	w.seeds.retainedSeeds = []retainedSeed{{taskID: "live-seed"}}

	w.seeds.ClearStaleReports(context.Background())
...
func TestCleanupRetainedSeedReportsStopped(t *testing.T) {
	...
	w.downloader = NewManagerWithDownloader(&recordingEngine{})
	cleaned := false
	w.seeds.retainedSeeds = []retainedSeed{{
		taskID:  "seed-task",
		engine:  "aria2",
		cleanup: func(context.Context) error { cleaned = true; return nil },
	}}

	w.seeds.cleanupRetainedSeed(context.Background(), w.seeds.retainedSeeds[0], "expired")
...
func TestCleanupRetainedSeedsKeepsFailedUploadLocalResult(t *testing.T) {
	...
	w := NewTaskRunnerWithAPI(config.Config{SeedEnabled: true, SeedCacheLimit: 1}, api)
	w.logger = slog.New(slog.NewTextHandler(io.Discard, nil))
	w.downloader = NewManagerWithDownloader(&recordingEngine{})
	cleaned := false
	seedDir := t.TempDir()
	...
	if len(w.seeds.retainedSeedSnapshot()) != 1 {
		t.Fatalf("expected retained seed to remain tracked, got %d", len(w.seeds.retainedSeedSnapshot()))
	}
...
func TestReconcileEngineSeedsDoesNotFetchLocalResultTasksWithoutLocalSeeds(t *testing.T) {
	...
	w.seeds.Reconcile(context.Background())
...
func TestReconcileEngineSeedsAdoptsUntrackedOrphans(t *testing.T) {
	...
	eng := &recordingEngine{listSeeds: []Seed{
	...
	w.logger = slog.New(slog.NewTextHandler(io.Discard, nil))
	w.downloader = NewManagerWithDownloader(eng)
	w.seeds.retainedSeeds = []retainedSeed{{taskID: "tracked-task"}}
	w.running["running-task"] = func(error) {}

	w.seeds.Reconcile(context.Background())

	for _, seed := range w.seeds.retainedSeedSnapshot() {
...
func TestHeartbeatReportsAggregateTransferSpeeds(t *testing.T) {
	w := NewTaskRunnerWithAPI(config.Config{Engine: "auto", MaxConcurrentTasks: 5, DownloadDir: t.TempDir()}, &recordingAPI{})
...
func TestCleanupDownloadedResultRemovesTaskDirForNestedDirectoryResult(t *testing.T) {
	...
	err := cleanupDownloadedResult(context.Background(), clientTaskWithStatus("task-1", "downloading"), Result{
		Path:  resultDir,
		Name:  "payload",
		IsDir: true,
	})
...
func TestDirectoryUploadFailureDeletesRemoteRootFolder(t *testing.T) {
	api := &recordingAPI{
		createFolderDrafts: []client.ObjectDraft{{ID: "root-folder", Name: "album"}},
		createObjectDraft:  client.ObjectDraft{ID: "file-object"},
	}
	w := NewTaskRunnerWithAPI(config.Config{}, api)
	resultPath := t.TempDir()
	if err := os.WriteFile(filepath.Join(resultPath, "track.flac"), []byte("audio"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !reflect.DeepEqual(api.deletedObjects, []string{"root-folder"}) {
		t.Fatalf("expected remote root folder cleanup, got %#v", api.deletedObjects)
	}
	failed := api.patches[len(api.patches)-1]
	if failed.State() != "failed" {
		t.Fatalf("expected failed status, got %q", failed.State())
...
func TestWorkerLifecycleUploadFailurePreservesLocalResult(t *testing.T) {
	...
	eng := &recordingEngine{
		...
		taskSnapshot: TaskSnapshot{
			...
			Result: &Result{Path: payloadPath, Name: "payload.bin", Size: payloadSize},
		},
		taskFound: true,
	}

	first := NewTaskRunnerWithAPI(config.Config{}, api)
	first.downloader = NewManagerWithDownloader(eng)
...
func TestWorkerLifecycleHTTPUploadFailurePreservesLocalResult(t *testing.T) {
	...
	first := NewTaskRunnerWithAPI(config.Config{}, api)
	first.downloader = NewManagerWithDownloader(preservingHTTPDownloadEngine{dir: downloadDir})
	...
	second := NewTaskRunnerWithAPI(config.Config{}, api)
	second.downloader = NewManagerWithDownloader(preservingHTTPDownloadEngine{dir: downloadDir})
...
func TestUploadExistingResultIncompleteRuntimeResumesDownload(t *testing.T) {
	...
	api := &recordingAPI{}
	eng := &recordingEngine{
		taskSnapshot: TaskSnapshot{State: TaskStateDownloading, Downloaded: 10},
		taskFound:    true,
...
func TestUploadShutdownMarksTaskInterrupted(t *testing.T) {
	...
	w.uploadAndComplete(
		...
		slog.New(slog.NewTextHandler(io.Discard, nil)),
		clientTaskWithUploadToken("task-1", "downloading"),
		Result{Path: payloadPath, Name: "payload.bin", Size: int64(len("downloaded payload"))},
		nil,
	)
...
func TestRetainSeedKeepsDownloadedResult(t *testing.T) {
	...
	stateDir := t.TempDir()
	cleaned := false
	w := NewTaskRunnerWithAPI(config.Config{SeedEnabled: true, SeedDuration: time.Hour, StateDir: stateDir}, nil)
	...
	if len(w.seeds.retainedSeedSnapshot()) != 1 {
		t.Fatalf("expected one retained seed, got %d", len(w.seeds.retainedSeedSnapshot()))
	}
...
func TestReportRetainedSeedsCleansMissingSeed(t *testing.T) {
	cleaned := false
	w := NewTaskRunnerWithAPI(config.Config{SeedEnabled: true}, &recordingAPI{})
	w.downloader = NewManagerWithDownloader(&recordingEngine{})
	w.seeds.retainedSeeds = []retainedSeed{{
		taskID: "task-1",
		engine: "aria2",
		seedID: "missing-gid",
		snapshot: func(context.Context) (SeedSnapshot, error) {
			return SeedSnapshot{}, errors.New("GID missing-gid is not found")
		},
		cleanup: func(context.Context) error {
			cleaned = true
			return nil
	...
	w.seeds.Report(context.Background())

	if !cleaned {
		t.Fatal("expected missing retained seed to be cleaned")
	}
	if got := len(w.seeds.retainedSeedSnapshot()); got != 0 {
		...
		snapshot: func(context.Context) (SeedSnapshot, error) {
			return SeedSnapshot{
				...
				Runtime: &TaskRuntime{
					...
					Phase:      "seeding",
					ETASeconds: &eta,
					Seeding:    &SeedingRuntime{UploadedBytes: &uploaded},
				},
			}, nil
		},
...
func TestReportRetainedSeedsStoppedClearsSeedingPhase(t *testing.T) {
		...
		snapshot: func(context.Context) (SeedSnapshot, error) {
			return SeedSnapshot{
				...
				Runtime: &TaskRuntime{
					Engine:  "aria2",
					Phase:   "seeding",
					Seeding: &SeedingRuntime{Active: &active, UploadedBytes: &uploaded},
				},
			}, nil
		},
		cleanup: func(context.Context) error { return nil },
	...
	w.seeds.ReportStopped(context.Background())
...
func TestRestoreRetainedSeedsLoadsLedger(t *testing.T) {
	...
	eng := &recordingEngine{name: "aria2", restoreSeed: &Seed{
		Engine:   "aria2",
		ID:       "new-gid",
		InfoHash: "abc123",
		Path:     seedPath,
	...
	w := NewTaskRunnerWithAPI(config.Config{SeedEnabled: true, StateDir: stateDir}, nil)
	w.downloader = NewManagerWithDownloader(eng)

	w.seeds.Restore(context.Background())

	seeds := w.seeds.retainedSeedSnapshot()
	if len(seeds) != 1 {
...
func TestRestoreRetainedSeedsDoesNotDuplicateAlreadyRestoredSeed(t *testing.T) {
	...
	if got := len(w.seeds.retainedSeedSnapshot()); got != 1 {
...
func TestCleanupRetainedSeedsRemovesExpiredSeed(t *testing.T) {
	...
	if len(w.seeds.retainedSeedSnapshot()) != 0 {
		t.Fatalf("expected retained seed to be removed, got %d", len(w.seeds.retainedSeedSnapshot()))
	}
}

func TestCleanupRetainedSeedsRemovesSeedAfterRatio(t *testing.T) {
	dir := t.TempDir()
	w := NewTaskRunnerWithAPI(config.Config{SeedEnabled: true, SeedRatio: 1.5}, &recordingAPI{})
	cleaned := false
	uploaded := int64(151)
	w.seeds.retainedSeeds = []retainedSeed{{
		taskID:     "task-1",
...
func TestCleanupRetainedSeedsRemovesOldestWhenCacheLimitExceeded(t *testing.T) {
	...
	var cleaned []string
	w := NewTaskRunnerWithAPI(config.Config{SeedEnabled: true, SeedCacheLimit: 3}, &recordingAPI{})
	w.seeds.retainedSeeds = []retainedSeed{
		{
			taskID:     "old",
			engine:     "qbittorrent",
...
type recordingEngine struct {
	...
	downloadErr    error
	downloadFunc   func(context.Context, DownloadTask, ProgressReporter) (Result, error)
	resetErr       error
	resetTaskFn    func(context.Context, DownloadTask) error
	taskSnapshot   TaskSnapshot
	inspectErr     error
...
type preservingHTTPDownloadEngine struct {
	dir string
}

func (e preservingHTTPDownloadEngine) Name() string {
	return "recording-http"
}

func (e preservingHTTPDownloadEngine) Capabilities() Capabilities {
	return Capabilities{SourceTypes: []string{"http"}}
}

func (e preservingHTTPDownloadEngine) Start(context.Context) error {
	return nil
}

func (e preservingHTTPDownloadEngine) Stop(context.Context) error {
	return nil
}

func (e preservingHTTPDownloadEngine) Check(context.Context) error {
	return nil
}

func (e preservingHTTPDownloadEngine) InspectTask(_ context.Context, task DownloadTask) (TaskSnapshot, bool, error) {
	path := filepath.Join(e.dir, task.ID, task.Name())
	info, err := os.Stat(path)
	if err != nil {
	...
	size := info.Size()
	result := Result{Path: path, Name: task.Name(), Size: size}
	return TaskSnapshot{
		State:      TaskStateCompleted,
		Downloaded: size,
		Total:      &size,
		Runtime:    &TaskRuntime{Engine: "recording-http", Phase: "completed"},
		Result:     &result,
	}, true, nil
}

func (e preservingHTTPDownloadEngine) Download(ctx context.Context, task DownloadTask, progress ProgressReporter) (Result, error) {
	res, err := http.Get(task.SourceURI())
	if err != nil {
		return Result{}, err
	}
	...
	dir := filepath.Join(e.dir, task.ID)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return Result{}, err
	}
	...
	size, copyErr := io.Copy(file, res.Body)
	closeErr := file.Close()
	if copyErr != nil {
	...
	total := size
	if err := progress(ProgressUpdate{
		Downloaded: size,
		Total:      &total,
		Runtime:    &TaskRuntime{Engine: "recording-http", Phase: "completed"},
	}); err != nil {
...
func (e *recordingEngine) Capabilities() Capabilities {
	return Capabilities{SourceTypes: []string{"http", "magnet", "torrent", "torrent_url"}}
}

func (e *recordingEngine) Start(context.Context) error {
	return nil
}

func (e *recordingEngine) Stop(context.Context) error {
	return nil
}
...
func (e *recordingEngine) InspectTask(context.Context, DownloadTask) (TaskSnapshot, bool, error) {
	e.inspectCalls++
	if e.inspectPanic != nil {
		panic(e.inspectPanic)
	}
	if e.inspectErr != nil {
		return TaskSnapshot{}, false, e.inspectErr
	}
	return e.taskSnapshot, e.taskFound, nil
}

func (e *recordingEngine) RestoreSeed(context.Context, SeedRef) (*Seed, error) {
	e.restoreCalls++
	return e.restoreSeed, e.restoreErr
}

func (e *recordingEngine) ListSeeds(context.Context) ([]Seed, error) {
	e.listSeedsCalls++
	return e.listSeeds, e.listSeedsErr
}

func (e *recordingEngine) ResetTask(ctx context.Context, task DownloadTask) error {
	e.resetCalls++
...
func (e *recordingEngine) Download(ctx context.Context, task DownloadTask, progress ProgressReporter) (Result, error) {
	e.downloadCalls++
...
func (a *recordingAPI) Heartbeat(context.Context, client.Heartbeat) (client.HeartbeatResult, error) {
	if a.heartbeatErr != nil {
		return client.HeartbeatResult{}, a.heartbeatErr
	}
...
func (a *recordingAPI) LocalResultTasks(context.Context) ([]client.DownloadTask, error) {
	...
	if a.localResultErr != nil {
		return nil, a.localResultErr
	}
...
func (a *recordingAPI) UpdateTask(_ context.Context, id string, patch client.TaskPatch) (client.DownloadTask, error) {
	if a.updateErr != nil {
		return client.DownloadTask{}, a.updateErr
	}
...
func (a *recordingAPI) CreateFolder(context.Context, string, string, string) (client.ObjectDraft, error) {
	if a.createFolderErr != nil {
		return client.ObjectDraft{}, a.createFolderErr
	}
	if len(a.createFolderDrafts) > 0 {
		draft := a.createFolderDrafts[0]
		a.createFolderDrafts = a.createFolderDrafts[1:]
		return draft, nil
	}
	return client.ObjectDraft{ID: "folder-1", Name: "folder"}, nil
}
...
func (a *recordingAPI) DeleteObject(_ context.Context, _ string, id string) error {
	a.deletedObjects = append(a.deletedObjects, id)
	return nil
}
