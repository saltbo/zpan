...
import (
	...
	"github.com/saltbo/zpan/pkg/downloaders/aria2"
	"github.com/saltbo/zpan/pkg/downloaders/httpdl"
	"github.com/saltbo/zpan/pkg/downloaders/qbittorrent"
)

func liveTask(id, sourceType, sourceURI, name string) downloader.DownloadTask {
	return downloader.DownloadTask{
		ID:          id,
		Source:      downloader.Source{Type: sourceType, URI: sourceURI},
		Destination: downloader.Destination{Name: name},
		Labels:      downloader.Labels{Tags: []string{}},
	}
}
...
func TestLiveDownloadThreeSourceTypes(t *testing.T) {
	...
	httpResult := runLiveDownload(
		t,
		ctx,
		httpdl.HTTP{Dir: filepath.Join(root, "http")},
	...
	magnetResult := runLiveDownload(
		t,
		ctx,
		&aria2.Aria2{URL: fmt.Sprintf("ws://127.0.0.1:%d/jsonrpc", rpcPort), Dir: filepath.Join(root, "magnet")},
		liveTask("live-magnet", "magnet", magnetURL, "magnet-fixture.txt"),
	)
	torrentResult := runLiveDownload(
		t,
		ctx,
		&aria2.Aria2{URL: fmt.Sprintf("ws://127.0.0.1:%d/jsonrpc", rpcPort), Dir: filepath.Join(root, "torrent-url")},
...
func TestLiveQBittorrentDownloadTorrentURL(t *testing.T) {
	...
	result := runLiveDownload(t, ctx, &qbittorrent.QBittorrent{
...
func runLiveDownload(t *testing.T, ctx context.Context, engine downloader.Downloader, task downloader.DownloadTask) downloader.Result {
	t.Helper()
	var lastDownloaded int64
	result, err := engine.Download(ctx, task, func(update downloader.ProgressUpdate) error {
		if update.Downloaded < lastDownloaded {
			t.Fatalf("download progress moved backwards: %d < %d", update.Downloaded, lastDownloaded)
		}
		lastDownloaded = update.Downloaded
		return nil
	})
	if err != nil {
		t.Fatal(err)
...
`, filepath.ToSlash(filepath.Join(profileDir, "downloads")), webUIPort)
	...
	engine := qbittorrent.QBittorrent{URL: "http://127.0.0.1:" + strconv.Itoa(webUIPort), Dir: t.TempDir()}
