...
func init() {
	downloader.Register("qbittorrent", false, configured, New)
}

func configured(cfg downloader.Config) bool {
	return cfg.QBittorrent.Configured
}
...
func New(cfg downloader.Config) (downloader.Downloader, error) {
	return &QBittorrent{
		...
		Managed:    !cfg.QBittorrent.Configured && (cfg.Engine == "" || cfg.Engine == "auto" || cfg.Engine == "qbittorrent"),
		GeoIP:      cfg.GeoIP,
	}, nil
}
...
func (q QBittorrent) Capabilities() downloader.Capabilities {
	return downloader.Capabilities{SourceTypes: []string{"magnet", "torrent", "torrent_url"}}
}

func (q *QBittorrent) Start(ctx context.Context) error {
	if !q.Managed {
		return nil
	}
	path, err := system.LookPathAny("qbittorrent-nox", "qbittorrent")
	if err != nil {
		return err
	}
...
func (q *QBittorrent) Stop(ctx context.Context) error {
	if q.cmd == nil {
		return nil
	}
	var errs []error
	if q.cmd.Process != nil {
		if err := q.cmd.Process.Kill(); err != nil {
			errs = append(errs, fmt.Errorf("kill qbittorrent process: %w", err))
		}
	}
...
func (q QBittorrent) startArgs(path string) ([]string, error) {
	webURL, err := system.ParseLocalEngineURL(q.URL, "8080")
	if err != nil {
		return nil, err
	}
	args := []string{}
	if q.StateDir != "" {
		...
		trackers := core.BTTrackers(q.BtTrackers)
		if err := writeQBittorrentManagedConfig(profileDir, q.Dir, webURL.Port, listenPortString(q.ListenPort), trackers); err != nil {
			return nil, err
		}
		args = append(args, "--profile="+profileDir)
	}
	if strings.Contains(system.FilepathBase(path), "qbittorrent-nox") {
		args = append(args, "--webui-port="+webURL.Port)
	}
	return args, nil
}

func writeQBittorrentManagedConfig(profileDir string, downloadDir string, webUIPort string, listenPort string, trackers string) error {
...
WebUI\Port=%s
`, listenPort, filepath.ToSlash(downloadDir), strings.ReplaceAll(core.BTTrackersForQBittorrent(trackers), "|", `\n`), webUIPort)
...
func (q QBittorrent) ResetTask(ctx context.Context, task downloader.DownloadTask) error {
	if task.SourceType() == "http" {
		return httpdl.HTTP{Dir: q.Dir}.ResetTask(ctx, task)
	}
	qbt, err := q.login(ctx)
	if err != nil {
		return err
...
func (q QBittorrent) RestoreSeed(ctx context.Context, ref downloader.SeedRef) (*downloader.Seed, error) {
	qbt, err := q.login(ctx)
	if err != nil {
		return nil, err
	}
...
func (q QBittorrent) InspectTask(ctx context.Context, task downloader.DownloadTask) (downloader.TaskSnapshot, bool, error) {
	if task.SourceType() == "http" {
		return httpdl.HTTP{Dir: q.Dir}.InspectTask(ctx, task)
	}
	qbt, err := q.login(ctx)
	if err != nil {
...
func (q QBittorrent) Download(ctx context.Context, task downloader.DownloadTask, progress downloader.ProgressReporter) (downloader.Result, error) {
	if task.SourceType() == "http" {
		return httpdl.HTTP{Dir: q.Dir}.Download(ctx, task, progress)
	}
	...
	options := q.qbittorrentAddOptions(task, taskDir, tag)
...
func qbittorrentTaskState(torrent qbittorrent.Torrent) downloader.TaskState {
	total := torrent.TotalSize
	if total <= 0 {
		total = torrent.Size
	}
...
func (q QBittorrent) findTask(ctx context.Context, qbt *qbittorrent.Client, task downloader.DownloadTask) (qbittorrent.Torrent, bool, error) {
	tag := qbittorrentTrackingTag(task.ID)
...
func (q QBittorrent) qbittorrentAddOptions(task downloader.DownloadTask, taskDir string, trackingTag string) map[string]string {
	category := "zpan"
	if task.Category() != "" {
	...
	if name := core.RequestedOutputName(task); name != "" {
		options["rename"] = name
	}
	if trackers := core.BTTrackersForQBittorrent(core.BTTrackers(q.BtTrackers)); trackers != "" {
...
func (q QBittorrent) seedSnapshot(hash string) func(context.Context) (downloader.SeedSnapshot, error) {
	return func(ctx context.Context) (downloader.SeedSnapshot, error) {
		...
		if len(torrents) == 0 {
			return downloader.SeedSnapshot{}, fmt.Errorf("qbittorrent torrent %s not found", hash)
		}
		torrent := torrents[0]
		total := torrent.TotalSize
		if total <= 0 {
	...
	for {
		...
		case <-ticker.C:
			...
			if err := progress(downloader.ProgressUpdate{
				...
				Total:      totalPtr,
				Bps:        torrent.DlSpeed,
				Runtime:    qbittorrentDetail(ctx, qbt, torrent, geoIP),
			}); err != nil {
...
func qbittorrentTrackers(ctx context.Context, qbt *qbittorrent.Client, torrent qbittorrent.Torrent) []downloader.Tracker {
...
func qbittorrentPeers(ctx context.Context, qbt *qbittorrent.Client, hash string, geoIP geoip.Resolver) []downloader.Peer {
	if hash == "" {
		return nil
	}
	...
	out := make([]downloader.Peer, 0, min(len(peers.Peers), 20))
	for address, peer := range peers.Peers {
		...
		core.ApplyPeerRegion(&item, peer.IP, peer.CountryCode, geoIP)
		out = append(out, item)
		if len(out) >= 20 {
			break
		}
...
func qbittorrentFiles(ctx context.Context, qbt *qbittorrent.Client, torrent qbittorrent.Torrent) []downloader.File {
	if torrent.Hash == "" {
		return nil
	}
	...
	for _, file := range *files {
		...
		out = append(out, downloader.File{
			Path:           core.StripTorrentRoot(file.Name, torrent.Name),
			Size:           file.Size,
...
func resultFromQBittorrentFiles(
	...
	files, err := qbt.GetFilesInformationCtx(ctx, torrent.Hash)
	if err != nil || files == nil {
		return core.ResultFromPath(task, taskDir, torrent.Name)
	}
	...
	for _, file := range *files {
		...
		abs, rel := core.DownloadedPath(taskDir, file.Name)
		downloaded = append(downloaded, core.DownloadedFile{Path: abs, RelativePath: rel})
	}
	if len(downloaded) == 0 {
		return core.ResultFromPath(task, taskDir, torrent.Name)
	}
	return core.ResultFromDownloadedFiles(task, taskDir, torrent.Name, downloaded)
}
