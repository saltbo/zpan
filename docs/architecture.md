...
explicit resource contracts and an exact path/method whitelist for the two
Downloader Device Flow operations generated by Better Auth. Better Auth's full
runtime schema remains available only from its own reference endpoints.
...
- `/api/openapi.json` — public ZPan product contract, including only the
  whitelisted Better Auth Downloader Device Flow operations
