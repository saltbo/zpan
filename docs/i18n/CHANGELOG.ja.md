## v2.9.0 — 2026-08-30 · 外部 OAuth アプリと Agent アクセス

### 新機能
- **外部 OAuth アプリケーション** — ZPan は、保護対象リソースと認可サーバーのメタデータ、動的クライアント登録、PKCE、Rich Authorization Requests、ワークスペース対応の同意、DPoP バインドアクセストークン、トークン交換、JWT 失効を備えた、検出可能な OAuth 2.1 リソースになりました。
- **アプリケーションと Grant の管理** — 管理者はインスタンス上で動的登録された全アプリを確認でき、ユーザーはアプリ名とクライアント ID、完全な scope 詳細、実際の最終使用日時を確認し、ワークスペース単位の Grant を取り消せます。
- **自己記述型 Agent API** — 正規 OpenAPI 文書が OAuth scope と安定した operation ID を公開し、Arazzo がアップロードのライフサイクルを記述します。オブジェクト作成レスポンスは汎用コントローラー向けに、単一および multipart upload の正式な手順を返します。
- **Realmroot 連携** — Agent Skills Discovery から、digest 固定の任意 `use-zpan` Skill、ワークスペース Context、最小権限の委任アクセスを公開します。Agent が作成したファイルやダウンロードには、表示と監査に使う実行者 ID が保持されます。
- **統一認可** — ブラウザーセッション、API キー、ダウンローダー、委任 OAuth プリンシパルは、共通の scope、ワークスペース、ロール、リソース状態ポリシーを通過します。OAuth が通常の管理者制約を迂回することはありません。
- **Agent 容量購入** — 委任アップロードは、コントローラー承認の x402 容量購入と永続的・テナント単位の冪等性により、クォータ不足から復旧できます。
- **画像用カスタムドメイン** — カスタムドメインプロバイダーの設定と検証、Cloudflare hostname の自動設定、Workers と Node の両方で正しい公開画像リンクを生成できます。

### 改善と修正
- 無効な DPoP proof に対する明示的な challenge とサーバー診断を追加し、サインイン中の OAuth 継続、公式 Worker preview origin、登録クライアント scope の同期、複数ワークスペース同意を修正しました。
- OAuth Grant の利用日時を実際の認証リクエストから、DB 書き込みを抑制しつつ更新します。取り消された Grant は、有効期間内のトークンも直ちに無効化します。
- Cloudflare Workers の S3 コピーで署名済み `x-amz-copy-source` ヘッダーを保持し、アーカイブ処理、アプリ一覧、同意表示のリリース回帰も修正しました。
- 実行者を識別できる監査表示とオンデマンドの作成者プロフィール、長いメタデータの制約、統一されたファイル操作メニューを追加しました。
- WebDAV、ファイル閲覧、ダウンロード、認証のホットパスで DB 往復を削減し、階層キャッシュを追加しました。一覧は安定したカーソルページングとリアルタイム更新を使用します。
- Worker キャッシュ初期化、ダウンローダーの bootstrap credential、容量購入の復旧、API キー永続化、CI 分離、Docker 検査、TypeScript 7 ツールチェーンを強化しました。

> **互換性に関する注意：** 新しい永続 ID は不透明な Base62 値です。API クライアントは ID を解析せず、既存環境では履歴データの統一が必要な場合のみ任意の正規化を実行してください。リソース API は正規 REST ルートと構造化エラーを使用します。最終 OAuth 契約では、固定 ZPan クライアント、Restish profile/plugin、credential helper、非標準 authorization-constraints 拡張を意図的に削除しました。

[完全なリリースノート ↗](https://github.com/saltbo/zpan/releases/tag/v2.9.0)

## v2.8.3 — 2026-08-30

### 修正
- ワークスペースのバックグラウンドジョブ操作を編集者と管理者に限定。
- ダウンローダー credential の認可を一元化し、Docker GeoIP データベースの取得元を更新。
- リリース時のセキュリティ勧告を解消するため依存関係を更新。

[完全なリリースノート ↗](https://github.com/saltbo/zpan/releases/tag/v2.8.3)

## v2.8.2 — 2026-08-03

### セキュリティ
- 送信リクエストの SSRF 保護を迂回し得る IPv6 移行アドレスの派生形を遮断。
- 通知メール内のユーザー制御コンテンツをエスケープ。
- リリース依存関係を更新し、強化した認証スタックから OpenAPI クライアントを再生成。

[完全なリリースノート ↗](https://github.com/saltbo/zpan/releases/tag/v2.8.2)

## v2.8.1 — 2026-07-24

### パフォーマンス
- データベース経由のリクエストフローにおける D1 の書き込み増幅を削減。

[完全なリリースノート ↗](https://github.com/saltbo/zpan/releases/tag/v2.8.1)

## v2.8.0 — 2026-07-24 · 管理分析と運用

### 新機能
- **管理分析ダッシュボード** — ストレージ、トラフィック、ユーザー、共有、
  リモートダウンロード、バックグラウンドジョブを対象としたライブ運用カードと
  信頼性の高い時間単位ロールアップ。トレンド、カバレッジ表示、バックフィル
  ツールも備えます。
- **ストレージ運用** — プロバイダープリセット、接続テスト、リクエストプレビュー、
  カテゴリ別使用量予測、ファイル配置追跡、安全なクリーンアップを備えた管理画面。
- **スペースとクォータ** — ワークスペース設定をメンバー、サービス、請求、
  画像ホスティングに整理し、クォータ所有権とスペース間操作を明確化。
- **公開プロフィールと共有** — 公開ページを刷新し、厳選した共有をユーザーの
  プロフィールへ直接掲載できます。
- **WebDAV 管理** — サービス制御と任意のカスタムドメイン、自動導出・検証に対応。
- **API とイベント** — Scalar ドキュメント付きの完全な OpenAPI、統一された
  リソーススキーマとエラー、フロントエンドのポーリングを置き換える SSE ストリーム。
- **その他の改善** — 管理者向けユーザー/チーム活動、監査フィルター、
  リモートダウンロード履歴、NFO プレビュー、メール認証設定、前回のログイン方法の記憶。

### 修正
- リモートダウンローダーの監視、heartbeat 復旧、シード、再試行、
  クレジット事前承認、進捗報告、クリーンアップを全面的に改善。
- 分析は範囲が確定した完了済みロールアップのみを使用し、不完全な履歴を
  部分データやライブデータと混在させず明示するようになりました。
- 非 ASCII ファイル名のアップロードメタデータと、アップロード後の保存済み
  Content-Type 同期を修正。
- Finder の WebDAV 互換性、使用中フォルダの保護、ダイアログのオーバーフロー、
  フォルダツリーの折りたたみを改善。

> **破壊的変更：** API のパスとレスポンスをリソース指向に統一しました。
> エラー形式も統一され、削除は `204 No Content` を返し、状態遷移には
> リソースの status エンドポイントを使用します。ランディングページ形式の共有は、
> 明示的に非公開へ設定しない限り、デフォルトで公開アクセス可能になりました。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.8.0)

## v2.7.4 — 2026-06-11

### 新機能
- `TRUSTED_ORIGINS` なしで loopback と LAN のオリジンを自動的に信頼。
- クラウドライセンスのペアリングで設定、エラー、確認手順を明確化。
- チェックアウト時にクーポンコードを入力可能。

### 修正
- リクエスト間のセッション初期化停止を解消し、高速なクライアントキャッシュを追加。
- 非 ASCII のアップロードファイル名を `Content-Disposition` で安全に処理。
- 重複チェックアウト時に古い保留中の注文を自動キャンセル。
- 管理フォームでデフォルトのクォータ値を正しく処理。
- D1 クエリ失敗時に根本原因のチェーンを記録。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.7.4)

## v2.7.3 — 2026-06-09

### 新機能
- **About ページ** — インスタンス情報・エディション・バージョンを表示する
  新しい管理者向け About ページ。更新履歴ドロワーを内蔵し、GitHub Releases
  に対して最新バージョンをチェックします。
- **商用ライセンス** — 独立した商用ライセンスに対応し、管理レイアウトに
  エディションのリボンを追加。機能ごとにグループ化したエディション比較表
  （ソーシャルログインとダウンローダーのゲート付き）を備えます。
- **エンタイトルメント管理** — 管理者が付与済みのクォータエンタイトルメントを
  編集・取り消しできるようになりました。
- **インスタンステレメトリ** — デプロイ情報（GeoIP リージョン付き）を任意かつ
  匿名で送信し、ZPan の運用状況を把握できるようにします。

### 修正
- リモートダウンロードの使用量課金をより堅牢に。
- Docker イメージの起動を修正（CI で保護）し、ダウンローダー登録にホストの
  ホスト名を使用するようにしました。
- 管理者向けクォータ一覧を高速化 —— バッチクエリ（D1 の 100 パラメータ上限
  に合わせて分割）を採用し、月次リセットをスケジュールジョブへ移行。
- アプリのバージョンを `package.json` から解決し、ビルド時に注入するように
  しました。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.7.3)

## v2.7.2 — 2026-06-07

### 新機能
- ZPan のロゴとブランディングを刷新。

### 修正
- リモートダウンローダーのデータボリュームが Docker で書き込み可能になりました。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.7.2)

## v2.7.1 — 2026-06-07

### 新機能
- 管理 UI からリモートダウンローダーの名前を変更できるようになりました。

### 修正
- ダウンローダーの割り当てをより確実にし、転送速度のレポートを正確化。
- torrent のリッスンポートを公開し、Docker のダウンローダーでホストのホスト名を使用するようにしました。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.7.1)

## v2.7.0 — 2026-06-06 · リモートダウンロード、WebDAV ほか

### 新機能
- **リモートダウンロードマネージャー** — torrent/HTTP ダウンロードをリモートワーカーへオフロードし、
  詳細なタスクインスペクター、ピアの地理的リージョン、BT のシード保持、
  ドライブへフォルダ構造を保ったままアップロードする機能を提供。
- **`zpan` CLI ダウンローダー** — ワンコマンドのデバイスログインと設定可能なサーバー URL。
- **WebDAV アクセス** — ユーザーごとのアプリパスワードで WebDAV 経由でドライブをマウント
  （RFC 4918 Class 2 互換）。
- **サーバーサイドアーカイブ** — ストリーミング ZIP ジョブをキューに入れ、新しい
  バックグラウンドタスクページで追跡。
- ウェブ UI での**フォルダアップロード**。
- **クラウドクレジット** — クレジットで課金される従量制のストレージ送信、クレジットストア付き。
- サインインとサインアップ向けの **Captcha** 保護。
- 統一された API キー管理。

### 修正
- リモートダウンロードのライフサイクル（リセット、リカバリー、シード処理）を堅牢化し、
  プレビューとアップロードに関する各種修正を実施。

> **破壊的変更：** より厳格な RESTful API ルート。公開ダウンロードリンクが
> `/dl/:token` から `/r/:token` へ移動。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.7.0)

## v2.6.2 — 2026-05-11

### 新機能
- 管理：クラウド注文詳細ドロワー。

### 修正
- ストレージプランのテーブルレイアウト、ギフトカードのマスキング、チェックアウト/注文履歴ダイアログ。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.6.2)

## v2.6.1 — 2026-05-10

### 修正
- チェックアウトのリダイレクトフローと、サイドバーのストレージクォータ表示。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.6.1)

## v2.6.0 — 2026-05-10 · Pro ライセンスとクォータストア

### 新機能
- **Pro ライセンス** — インスタンスを ZPan Cloud とペアリング（QR + ペアリングモーダル）、
  Ed25519 で検証されたエンタイトルメントとバックグラウンド更新、そして Pro 機能のゲーティング。
- **ホワイトラベルブランディング** — カスタムロゴ、ファビコン、ワードマーク、フッターの非表示。
- **クォータストア** — 引き換えコード、月間トラフィッククォータ、サブスクリプションおよび
  固定クォータパッケージ、通貨ごとの従量制価格、トラフィック超過課金。
- **管理** — 状態を変更する操作にまたがる監査ログ、サイトのお知らせ、
  招待ベースのサインアップ、再設計された設定と概要ダッシュボード。
- ファイルプレビューに Microsoft Office ビューア、音楽プレーヤー、複数ファイルの
  アップロード進捗キューを追加。

### 修正
- Billing を管理パネルへ移動。管理画面でのクォータ単位、ユーザークォータ、アバター。
- クラウドのトラフィック使用量をバックグラウンド同期。ダウンロードリンクで月間トラフィックを強制。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.6.0)

## v2.5.0 — 2026-04-23 · どこへでもデプロイ

### 新機能
- **新しいデプロイ先** — AWS Lambda、Vercel、Netlify、Azure Functions、
  Google Cloud Run。
- **libSQL（Turso）** データベースアダプター、オプトイン式の Docker 構成付き。
- Settings → Profile でのアバターアップロード。
- 画像アップロードに Cloudflare R2 バインディングを優先し、S3 へフォールバック。

### 修正
- 設定タブ全体のビジュアルデザインを統一し、不足していたアバターの i18n を追加。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.5.0)

## v2.4.1 — 2026-04-22

### 修正
- ポート 8222 での Docker 404 を解消し、イメージビルドを簡素化。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.4.1)

## v2.4.0 — 2026-04-22 · 画像ホスティング

### 新機能
- **画像ホスティング** — 二段階/ストリームプロキシアップロード、カスタムドメイン
  （Cloudflare for SaaS）、設定ページを備えた専用ギャラリー。
- **ツール連携** — PicGo、uPic、ShareX 向けの既製設定。
- プログラムによるアップロードのための API キー認証。

### 修正
- PicGo / uPic / ShareX の設定、下書き画像のフィルタリング、
  大容量/マルチパートアップロードのエラーを修正。

> **破壊的変更：** 公開リンクを `/r/:token` の下に統一。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.4.0)

## v2.3.0 — 2026-04-21 · 共有

### 新機能
- **ファイル＆フォルダ共有** — ランディングモードとダイレクトモードを備えた
  公開共有ページ（`/s/:token`）、任意で自動生成されるパスワード、フォルダブラウジング。
- **ドライブへ保存** — クォータと名前の競合を処理しながら、ワークスペース間で
  共有ファイルをコピー。
- **アプリ内通知**と専用の Shares ダッシュボード。
- Google パレットの UI 再設計。通知ベルをヘッダーへ移動。

### 修正
- Finder スタイルの名前競合解決、共有パスワード誤りでの正しい 403、
  公開ビューカウントの重複排除。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.3.0)

## v2.2.0 — 2026-04-19 · チーム

### 新機能
- **チームワークスペース** — 組織レベルの RBAC でチーム、メンバー、ロールを
  作成・管理。
- サイドバーのワークスペース切り替えと、チームごとのアクティビティフィード。
- メールと招待リンクによる**チーム招待**。
- `/u/:username` の公開ユーザーホームページ。

### 修正
- チームリストのフィルタリングとメンバー数。Teams エントリをアバターメニューへ移動。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.2.0)

## v2.1.0 — 2026-04-14 · 認証とオンボーディング

### 新機能
- **動的 OAuth プロバイダー**、検証付きのメール/パスワード、設定可能な
  登録モード。
- **招待コード**による登録ゲーティング。
- メールサービスの抽象化（SMTP + HTTP API ドライバー）。
- サインイン/サインアップ UI の刷新と、管理者向け認証設定ページ。

### 修正
- 招待コードの検証と、サイドバーのダークモード描画。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.1.0)

## v2.0.2 — 2026-04-12

### 新機能
- デスクトップ、タブレット、モバイル向けのレスポンシブレイアウトと、適応的なモバイルプレビュー。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.0.2)

## v2.0.1 — 2026-04-12

### 新機能
- ワンクリックデプロイボタンを備えた Cloudflare Workers への移行。

### 修正
- Cloudflare Workers のデプロイと認証設定（baseURL/trusted
  origins の推論、`nodejs_compat_v2`）。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.0.1)

## v2.0.0 — 2026-04-12 · TypeScript への書き直し

### 新機能
- Go から TypeScript への完全な書き直し：Hono API + React SPA で、
  Cloudflare Workers と Node/Docker の両方にデプロイ可能。
- presigned URL による S3 への直接アップロード、フォルダツリー、検索、
  ゴミ箱を備えたカスタムファイルマネージャー。
- 画像、PDF、コード、音声、動画のファイルプレビュー。
- 管理者向けのユーザー/ストレージ/クォータ管理、組織ごとのストレージクォータ、i18n（en/zh）。

### 修正
- サーバーサイドのグローバル検索（Enter でトリガー）とメディアプレビューの描画。

[Full release notes ↗](https://github.com/saltbo/zpan/releases/tag/v2.0.0)

---

v1 の変更履歴については [v1 ブランチ](https://github.com/saltbo/zpan/tree/v1/CHANGELOG.md) を参照してください。
