...
### 기능
- **관리자 분석 대시보드** — 저장소, 트래픽, 사용자, 공유, 원격 다운로드,
  백그라운드 작업을 위한 실시간 운영 카드와 신뢰할 수 있는 시간별 롤업을
  제공하며 추세, 데이터 범위 표시, 백필 도구를 포함합니다.
- **저장소 운영** — 공급자 프리셋, 연결 테스트, 요청 미리보기, 범주별 사용량
  예측, 파일 위치 추적, 안전한 정리 흐름을 갖춘 새 백엔드 관리 화면.
- **공간과 할당량** — 워크스페이스 설정을 구성원, 서비스, 결제, 이미지 호스팅으로
  분리하고 할당량 소유권과 공간 간 파일 작업을 명확하게 정리했습니다.
- **공개 프로필과 공유** — 새 공개 페이지에서 선별한 공유를 사용자 프로필에
  직접 표시할 수 있습니다.
- **WebDAV 관리** — 서비스 제어와 선택적 사용자 지정 도메인, 자동 도메인
  파생 및 검증을 지원합니다.
- **API 및 이벤트** — Scalar 문서를 포함한 완전한 OpenAPI, 일관된 리소스
  스키마와 오류, 프런트엔드 폴링을 대체하는 통합 SSE 스트림.
- **추가 개선** — 관리자 사용자/팀 활동, 향상된 감사 필터, 원격 다운로드
  이벤트 타임라인, NFO 미리보기, 이메일 인증 설정, 마지막 로그인 방식 기억.

### 수정
- 원격 다운로더의 감독, heartbeat 복구, 시딩, 재시도, 크레딧 사전 승인,
  진행률 보고와 정리 기능을 전면 개선했습니다.
- 분석은 범위가 명확하고 완료된 롤업만 사용하며, 불완전한 이력을 부분 또는
  실시간 데이터와 섞지 않고 명확히 표시합니다.
- 비 ASCII 파일명의 업로드 메타데이터와 업로드 후 저장된 콘텐츠 유형 동기화를 수정했습니다.
- Finder WebDAV 호환성, 활성 폴더 보호, 대화상자 넘침, 폴더 트리 접기를 개선했습니다.

> **호환성이 깨지는 변경:** API 경로와 응답이 리소스 중심으로 통일되었습니다.
> 오류는 통합 형식을 사용하고 삭제는 `204 No Content`를 반환하며 상태 변경은
> 리소스 상태 엔드포인트를 사용합니다. 랜딩 페이지 공유는 명시적으로 비공개로
> 설정하지 않는 한 기본적으로 공개됩니다.
...
### 기능
- `TRUSTED_ORIGINS` 없이 loopback 및 LAN origin을 자동으로 신뢰합니다.
- 클라우드 라이선스 페어링의 구성, 오류, 확인 절차가 더 명확해졌습니다.
- 결제 중 쿠폰 코드를 입력할 수 있습니다.

### 수정
- 요청 간 세션 초기화 멈춤을 제거하고 더 빠른 클라이언트 세션 캐시를 추가했습니다.
- 비 ASCII 업로드 파일명을 `Content-Disposition`에서 안전하게 처리합니다.
- 중복 결제 시 오래된 대기 주문을 자동으로 취소합니다.
- 관리자 양식에서 기본 할당량 값을 올바르게 처리합니다.
- D1 쿼리 실패 시 근본 원인 체인을 기록합니다.
