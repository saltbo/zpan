## v2.9.0 — 2026-08-30 · 外部 OAuth 应用与 Agent 访问

### 新功能
- **外部 OAuth 应用** — ZPan 现在是可发现的 OAuth 2.1 资源，提供受保护资源与授权服务器元数据、动态客户端注册、PKCE、富授权请求、工作区感知授权、DPoP 绑定访问令牌、令牌交换和 JWT 撤销。
- **应用与授权管理** — 管理员可以查看实例上所有动态注册的应用；用户可以查看应用名称和客户端 ID、检查完整 scope 详情、查看真实的最后使用时间，并撤销单个工作区授权。
- **自描述 Agent API** — 规范 OpenAPI 文档发布 OAuth scope 和稳定的 operation ID，Arazzo 描述上传生命周期，创建对象响应为通用控制器提供权威的单段及分片上传指令。
- **Realmroot 集成** — Agent Skills Discovery 发布可选、固定摘要的 `use-zpan` Skill，支持工作区 Context 和最小权限委托访问。Agent 创建的文件与下载现在会保留执行者身份，用于用户侧归属展示和审计记录。
- **统一授权** — 浏览器会话、API 密钥、下载器和委托 OAuth 主体现在统一经过与协议无关的 scope、工作区、角色和资源状态策略；OAuth 不会绕过常规管理员约束。
- **Agent 容量购买** — 委托上传在配额不足时可通过控制器批准的 x402 容量购买恢复，并具备持久、租户隔离的幂等性。
- **自定义图床域名** — 可配置并验证自定义域名供应商，自动完成 Cloudflare 主机名设置，并在 Workers 与 Node 部署中生成正确的公开图片链接。

### 改进与修复
- 为无效 DPoP proof 增加明确挑战和服务端诊断，保留登录过程中的 OAuth 续接，信任官方 Worker 预览来源，同步已注册客户端的 scope，并修正多工作区授权处理。
- OAuth Grant 活动现在会根据真实认证请求更新，并限制数据库写入频率；撤销 Grant 后会立即使仍在有效期内的令牌失效。
- 修复 Cloudflare Workers 上的 S3 复制请求，保留已签名的 `x-amz-copy-source` 请求头；同时修复归档处理、应用列表和授权页面的发布门禁回归问题。
- 新增执行者感知的审计展示和按需加载的创建者资料，约束文件及资料界面的长元数据，并统一文件操作菜单。
- 减少 WebDAV、文件浏览、下载与认证热路径的数据库往返并加入分层缓存。列表页现在使用稳定的游标分页和实时更新，不再依赖前端轮询。
- 加固 Worker 缓存初始化、下载器引导凭据、容量购买恢复、API 密钥持久化、CI 隔离、Docker 检查和 TypeScript 7 工具链。

> **兼容性说明：** 新的持久化标识符采用不透明 Base62 值；API 使用方不得解析 ID，已有部署可在需要统一历史数据时运行可选的规范化流程。资源 API 使用规范 REST 路由和结构化错误。最终 OAuth 合约有意移除了固定 ZPan 客户端、Restish profile/plugin、凭据辅助工具以及非标准的 authorization-constraints 扩展。

[完整发布说明 ↗](https://github.com/saltbo/zpan/releases/tag/v2.9.0)

## v2.8.3 — 2026-08-30

### 修复
- 将工作区后台任务控制限制为编辑者和管理员。
- 集中管理下载器凭据授权，并更新 Docker GeoIP 数据库来源。
- 更新依赖以修复发布阶段的安全公告。

[完整发布说明 ↗](https://github.com/saltbo/zpan/releases/tag/v2.8.3)

## v2.8.2 — 2026-08-03

### 安全
- 阻止可能绕过出站请求 SSRF 防护的 IPv6 转换地址变体。
- 转义通知邮件中的用户可控内容。
- 更新发布依赖，并基于加固后的认证栈重新生成 OpenAPI 客户端。

[完整发布说明 ↗](https://github.com/saltbo/zpan/releases/tag/v2.8.2)
...
[完整发布说明 ↗](https://github.com/saltbo/zpan/releases/tag/v2.8.1)
