...
image_hostings
  ...
  orgId
  token UNIQUE              // "i" + 11 random Base62 characters; supports token URL access
...
|---|---|---|
| **Default app domain** (`zpan.io`) | Token URL only: `/r/:token` | `https://zpan.io/r/A7B3xK9Q2LmZ.png` |
...
Shared with direct shares (old `/d/:token` is removed outright; no alias is kept). Share tokens are `s` plus 11 random
Base62 characters; image tokens are `i` plus 11 random Base62 characters. After stripping one optional alphanumeric
extension, the route validates the 12-character token and selects exactly one repository from its first character:

| Record kind | Cache-Control | Content-Disposition |
|---|---|---|
| Direct share | `no-store` | `attachment` |
| Image hosting | `no-store` | `inline` |

Optional file extension for Markdown / browser hinting: `/r/iA7B3xK9Q2Lm.png`. Server ignores the extension when resolving — only the token is authoritative. Extension is derived from stored `mime` at upload.

Redirect responses are never cached, so clients cannot reuse an expired or revoked presigned URL.
...
- Auth: **better-auth `apiKey` plugin** (`Authorization: Bearer <key>`), not a custom token table. Keys are user-owned, carry a workspace scope in metadata plus an `image-hosting:upload` permission, and inherit the plugin's revocation / `lastUsedAt` / optional expiration machinery.
- Form fields:
...
```json
{
  "data": {
    ...
    "urlAlt":   "https://zpan.io/r/A7B3xK9Q2LmZ.png",
...
| Var | Description |
|-----|-------------|
Image custom-domain provisioning is configured by the site administrator in
**Admin → Settings → Image custom domains**. It is no longer controlled by
runtime environment variables. The administrator can connect an existing
Cloudflare for SaaS zone or use the self-managed DNS provider.
