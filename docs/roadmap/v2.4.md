...
image_hostings
  ...
  orgId
  token UNIQUE              // 12 random Base62 characters; supports token URL access
...
Shared with direct shares (old `/d/:token` is removed outright; no alias is kept). The database record kind disambiguates
the resource; tokens do not carry type prefixes:

| Record kind | Cache-Control | Content-Disposition |
|---|---|---|
| Direct share | `no-store` | `attachment` |
| Image hosting | `public, max-age=300` | `inline` |

Optional file extension for Markdown / browser hinting: `/r/A7B3xK9Q2LmZ.png`. Server ignores the extension when resolving — only the token is authoritative. Extension is derived from stored `mime` at upload.
