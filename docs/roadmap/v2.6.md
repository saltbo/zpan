...
- **Store Cloud communication is automatic through the Pro binding.** ZPan resolves Cloud from `ZPAN_CLOUD_URL` or the built-in default, authenticates store requests with the bound instance credentials, and derives the public instance origin from `ZPAN_PUBLIC_ORIGIN`, `BETTER_AUTH_URL`, or the request URL. Forwarded headers are not trusted for Cloud-facing store URLs.
- **Delivery auth uses current binding credentials.** Cloud must authenticate delivery attempts with the current active binding credential for the ZPan instance; stale credentials after binding refresh are rejected with `401`.
...
4. ZPan syncs the catalog to Cloud through `POST /api/store/packages/sync` using the active Pro binding credentials
...
7. ZPan verifies the binding-derived delivery auth, records the delivery event idempotently, appends one quota grant, and increases the target org's effective quota
