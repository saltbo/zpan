...
- **Terminal buyers do not need Cloud accounts.** ZPan creates purchase or redemption requests through the existing Pro Cloud binding and sends the user to Cloud only for checkout/redeem handling.
...
- **Store Cloud communication is automatic through the Pro binding.** ZPan resolves Cloud from `ZPAN_CLOUD_URL` or the built-in default, authenticates catalog sync, checkout, and redemption with the bound instance refresh token, and sends normal JSON request bodies that include `boundLicenseId`.
- **Delivery auth uses Cloud-signed event tokens.** Cloud delivery attempts include `x-zpan-cloud-event-token`, a PASETO token signed by Cloud. ZPan verifies it with the built-in Cloud public keys and requires the current issuer, an audience equal to the bound license id, the bound license id claim, delivery event id, raw-body payload hash, and short expiry.
...
3. Checkout calls `POST /api/quota-store/checkout`, which validates target access and creates a Cloud checkout using the Pro binding refresh token
...
7. ZPan verifies the Cloud event token and raw payload hash, records the delivery event idempotently, appends one quota grant, and increases the target org's effective quota
...
- The user selects the target org, submits the code, and ZPan sends a redemption request to Cloud through the Pro binding.
