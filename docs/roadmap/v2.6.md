...
8. Normal subscription renewals are billed in Cloud only; when they change the entitlement period, Cloud sends a replacement delivery that cannot stack quota
...
- Local monthly traffic quota is still enforced before Cloud reporting. Cloud metering is for credit-backed usage billing, not a replacement for local quota checks.
...
- **In-ZPan generic membership redemption input** — membership redemption lives on the cloud dashboard; only gift-card credit redemption is accepted in the ZPan quota store
