...
- **Delivery auth uses Cloud-signed event tokens.** Cloud delivery attempts include `x-zpan-cloud-event-token`, a PASETO token signed by Cloud. ZPan verifies it with the built-in Cloud public keys and requires the current issuer, instance audience, bound license id, delivery event id, raw-body payload hash, and short expiry.
