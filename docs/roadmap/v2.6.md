...
- The user selects the target org, enters the gift card during checkout, and ZPan sends one checkout request to Cloud through the Pro binding.
- Cloud validates gift-card balance and order state, then sends an order quota-change webhook through the same authenticated callback path.
...
**Terminal user applying a gift card:**

> I receive a gift card from the instance operator. In ZPan Store, I select the target team and apply the gift card during checkout. The order appears in my order history and the team's quota increases. This does not activate Pro membership and does not use the Cloud membership redemption page.
