...
The v2.9 API surface uses the unified authorization kernel documented in
[`docs/design/agent-authentication.md`](../design/agent-authentication.md).
Routes declare public/internal/protected status and canonical scopes once; that
declaration feeds both runtime authorization and OpenAPI metadata.

Canonical scopes are lowercase `plural-resource:action` authorities. Legacy
API-key permission JSON for image hosting, WebDAV, and remote downloads must be
backfilled with `pnpm api-key-scopes:backfill -- --apply ...` before deploying
code that only accepts canonical scopes. Standard OAuth device authorization for
third-party agents remains deferred to v2.9.x; this version establishes the
route contract and policy kernel it will use.
