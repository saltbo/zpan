...
- **Terminal buyers do not need Cloud accounts.** ZPan creates purchase and redemption sessions through the Pro Cloud binding; Cloud uses those sessions for checkout or storage-code redemption.
...
Quota store communication is automatic through the Pro Cloud binding. ZPan resolves Cloud from `ZPAN_CLOUD_URL` or the built-in default, sends catalog sync with `boundLicenseId` and the bound instance refresh token, and signs checkout/redemption sessions with the binding-derived `store_key` returned by Cloud entitlements. Forwarded headers are not trusted for Cloud-facing store URLs.

Cloud delivery attempts must include `x-zpan-cloud-timestamp` and `x-zpan-cloud-signature` signed with the current active binding `store_key` for the ZPan instance. Stale keys after entitlement refresh are rejected with `401`.
...
4. After Stripe marks the Cloud order paid, Cloud delivers the quota grant to ZPan through an authenticated callback.
...
- Cloud validates the storage code and sends quota delivery through the same authenticated callback path.
...
- Anonymous Cloud checkout handoff from binding-authenticated ZPan storage sessions
- Stripe-paid Cloud order delivery through authenticated callback
- Duplicate Cloud delivery returning success without a second grant
- Bad Cloud delivery auth returning `401`
...
3. Start checkout and confirm ZPan opens the Cloud/Stripe checkout URL from the storage session.
...
Backend verification covered duplicate Cloud delivery not double-granting quota, bad delivery auth returning `401`, and admin base quota edits preserving paid grants.
...
- `feat(quota-store)`: Authenticated Cloud delivery callback with idempotent quota grants
