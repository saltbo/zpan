...
- **Terminal buyers do not need Cloud accounts.** ZPan creates binding-authenticated purchase and redemption requests through the Pro Cloud binding; Cloud uses those requests for checkout or storage-code redemption.
...
Quota store communication is automatic through the Pro Cloud binding. ZPan resolves Cloud from `ZPAN_CLOUD_URL` or the built-in default and sends catalog sync, checkout, and storage-code redemption requests with `Authorization: Bearer <refresh_token>` plus JSON bodies that include `boundLicenseId` and the required business fields. Forwarded headers are not trusted for Cloud-facing store URLs.

Cloud delivery attempts must include `x-zpan-cloud-event-token`, a Cloud-signed PASETO event token. ZPan verifies the token with the built-in Cloud public keys and requires `type: "zpan.cloud.event"`, `purpose: "quota_store.delivery"`, the current Cloud issuer, an audience equal to the bound license id, the bound license id claim, the delivery event id, a raw-body SHA-256 payload hash, and a short expiry. Missing, expired, wrong-purpose, wrong-audience, wrong-hash, or mismatched-event tokens are rejected with `401`.
