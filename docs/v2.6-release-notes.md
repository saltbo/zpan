...
Quota store communication is automatic through the Pro Cloud binding. ZPan resolves Cloud from `ZPAN_CLOUD_URL` or the built-in default, derives the instance origin from `ZPAN_PUBLIC_ORIGIN`, `BETTER_AUTH_URL`, or the request URL, and authenticates store sync, checkout, redemption, and delivery with binding-derived credentials. Forwarded headers are not trusted for Cloud-facing store URLs.

Cloud delivery attempts must use the current active binding credential for the ZPan instance. Stale credentials after binding refresh are rejected with `401`.
