...
Cloud delivery attempts must include `x-zpan-cloud-event-token`, a Cloud-signed PASETO event token. ZPan verifies the token with the built-in Cloud public keys and requires `type: "zpan.cloud.event"`, `purpose: "quota_store.delivery"`, the current Cloud issuer, the current instance audience, the bound license id, the delivery event id, a raw-body SHA-256 payload hash, and a short expiry. Missing, expired, wrong-purpose, wrong-audience, wrong-hash, or mismatched-event tokens are rejected with `401`.
