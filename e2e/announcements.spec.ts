  ...
  test('admin publishes an announcement and user sees it in the announcement modal @desktop', async ({ page }) => {
    ...
    try {
      ...
      const [createResp] = await Promise.all([
        page.waitForResponse((r) => r.url().includes('/api/site/announcements') && r.request().method() === 'POST'),
      ...
      const [publishResp] = await Promise.all([
        page.waitForResponse((r) => r.url().includes('/api/site/announcements') && r.request().method() === 'PUT'),
