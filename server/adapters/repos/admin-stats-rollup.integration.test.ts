...
describe('admin hourly stats rollup', () => {
  ...
  it('rebuilds event, user, operational, and snapshot metrics idempotently', async () => {
    ...
    expect(
      rows.filter(
        (value) =>
          metricDefinition(value.metric as keyof typeof ADMIN_STATS_METRIC_REGISTRY).kind === 'gauge' &&
          value.orgId !== '',
      ),
    ).toEqual([])
    ...
    expect(row(M.storageTrashSnapshot)).toBeUndefined()
    ...
    expect(row(M.shareInventory)).toBeUndefined()
    ...
    expect(row(M.backgroundJobSnapshot)).toBeUndefined()
    expect(row(M.backgroundJobSnapshot, '', '', '')).toMatchObject({ count: 1 })
    expect(row(M.remoteDownloadTaskSnapshot)).toBeUndefined()
