...
describe('site stats routes', () => {
  ...
  it('repairs a direct-share duplicate before completed-hour traffic and sharing rollups are served', async () => {
    const { app, db } = await createTestApp()
    const headers = await adminHeaders(app)
    await seedProLicense(db)
    const { orgId, userId } = await seedStatsFixture(db)
    const bucketStart = Date.UTC(2026, 6, 1, 10)
    const eventAt = bucketStart + 60_000
    const eventAtSec = Math.floor(eventAt / 1000)
    ...
    await db.run(sql`
      ...
      VALUES (
        'hourly-direct-share', 'hourly-direct-share', 'direct', 'stats-file', ${orgId}, ${userId},
        NULL, 10, 0, 1, 'active', ${eventAtSec}
      )
    `)
    await db.run(sql`
      INSERT INTO activity_events (
      ...
      VALUES
        (
          'hourly-native-direct-share', ${orgId}, NULL, 'anonymous', NULL, 'share_download', 'share',
          'hourly-direct-share', 'report.pdf',
          '{"bytes":512,"source":"direct_share","status":"issued","anonymous":true}', ${eventAtSec}
        ),
        (
          'backfill_hourly-direct-share-traffic', ${orgId}, NULL, 'system', 'stats-backfill',
          'share_download', 'share', 'hourly-direct-share', 'hourly-direct-share',
          '{"direction":"download","status":"issued","source":"direct_share","bytes":512,"trafficEventId":"hourly-direct-share-traffic","quality":"recovered_from_cloud_traffic_report"}',
          ${eventAtSec}
        )
    `)
    await db.run(sql`
      ...
      VALUES (
        'hourly-direct-share-report', ${orgId}, '2026-07', 'direct_share', 'hourly-direct-share',
        NULL, 'hourly-direct-share-traffic', 512, NULL, NULL, 'reported', NULL, ${eventAt}, ${eventAt}
      )
    `)

    db.$client.exec(buildBackfillSql(new Date('2026-07-10T12:00:00.000Z')))
    ...
    const repairedRows = await db.all<{
      ...
      WHERE target_id = 'hourly-direct-share' AND action = 'share_download'
    `)
    const query = 'from=2026-07-01T10%3A00%3A00.000Z&to=2026-07-01T10%3A59%3A59.999Z&timeZone=UTC'
    const trafficRes = await app.request(`/api/site/stats/traffic?${query}`, { headers })
    ...
    const sharingRes = await app.request(`/api/site/stats/sharing?${query}`, { headers })
    ...
    expect(repairedRows).toEqual([
      {
        id: 'hourly-native-direct-share',
        trafficEventId: 'hourly-direct-share-traffic',
        source: 'direct_share',
        bytes: 512,
      },
    ])
    expect(trafficRes.status).toBe(200)
    expect(trafficBody.summary).toMatchObject({
      totalBytes: { value: 512 },
    ...
    expect(sharingBody.summary.downloads.value).toBe(1)
