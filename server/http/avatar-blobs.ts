...
import { AVATARS_BINDING, type R2BucketLike } from '../platform/interface'

// Public read of a self-hosted avatar blob from the AVATARS R2 binding. Only used on
// Cloudflare when the binding is present and AVATARS_PUBLIC_URL is NOT set (e.g. local
...
export async function serveAvatarBlob(c: Context<Env>) {
  const bucket = c.get('platform').getBinding<R2BucketLike>(AVATARS_BINDING)
