...
describe('GET /api/events', () => {
  ...
  it('streams durable resource changes for an authed user [spec: events/stream]', async () => {
    ...
    const res = await testApp.app.request('/api/events', { headers: { ...headers, 'Last-Event-ID': '0' } })
    expect(res.status).toBe(200)
    expect(res.headers.get('content-type')).toContain('text/event-stream')

    const text = await readSome(res, ['"resourceType":"background_job"', 'browser-org-task'])
    expect(text).toContain('event: resource-change')
    expect(text).toContain('"resourceType":"background_job"')
  ...
  it('streams only scoped download tasks to an authorized workspace API key [spec: events/api-key-download-tasks]', async () => {
    ...
    expect(res.status).toBe(200)
    expect(res.headers.get('content-type')).toContain('text/event-stream')
    const text = await readSome(res, ['event: resource-change'])
    expect(text).toContain('event: resource-change')
  ...
  it('does not require per-page opt-ins for an authorized workspace API key [spec: events/api-key-global-stream]', async () => {
    const testApp = await createTestApp()
  ...
  it('closes the stream when the request is aborted [spec: events/abort]', async () => {
    const testApp = await createTestApp()
    const { headers, orgId, userId } = await authedOrg(testApp)
    await createBackgroundJobRepo(testApp.db).create({ orgId, userId, type: 'archive_compress' })
    ...
    expect(decoder.decode(first.value)).toContain('event: resource-change')
  ...
  it('emits an error event when a domain query fails [spec: events/error-event]', async () => {
    ...
    vi.spyOn(testApp.deps.resourceChanges, 'listAfter').mockRejectedValueOnce(new Error('boom'))
