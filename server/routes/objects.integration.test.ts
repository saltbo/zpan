...
describe('Matter service', () => {
  ...
  it('POST /api/objects/batch/visibility sets items public', async () => {
    const { app, db } = await createTestApp()
    const headers = await authedHeaders(app)
    await insertStorage(db)
    const orgId = await getOrgId(db)
    await insertFile(db, orgId, { id: 'v1', name: 'a.txt' })
    await insertFile(db, orgId, { id: 'v2', name: 'b.txt' })

    const res = await app.request('/api/objects/batch/visibility', {
      method: 'POST',
      headers: { ...headers, 'Content-Type': 'application/json' },
      body: JSON.stringify({ ids: ['v1', 'v2'], isPublic: true }),
    })
    expect(res.status).toBe(200)
  ...
  it('POST /api/objects/batch/visibility sets items private', async () => {
    const { app, db } = await createTestApp()
    const headers = await authedHeaders(app)
    await insertStorage(db)
    const orgId = await getOrgId(db)
    await insertFile(db, orgId, { id: 'v3', name: 'c.txt' })

    const res = await app.request('/api/objects/batch/visibility', {
      method: 'POST',
      headers: { ...headers, 'Content-Type': 'application/json' },
      body: JSON.stringify({ ids: ['v3'], isPublic: false }),
    })
    expect(res.status).toBe(200)
    const body = (await res.json()) as { updated: number }
  ...
  it('POST /api/objects/batch/visibility returns 400 for invalid input', async () => {
    const { app } = await createTestApp()
    const headers = await authedHeaders(app)
    const res = await app.request('/api/objects/batch/visibility', {
      method: 'POST',
  ...
  it('POST /api/objects/batch/visibility returns 401 without auth', async () => {
    const { app } = await createTestApp()
    const res = await app.request('/api/objects/batch/visibility', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ ids: ['x'], isPublic: true }),
    })
    expect(res.status).toBe(401)
  })
})
